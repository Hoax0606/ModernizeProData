/**
 * FontAwesome Free 7.2 hourglass-half solid (path 그대로).
 * pending snapshot 표시용 — Project 이름 옆에 amber 색으로 등장.
 * fill="currentColor" 라서 부모 color 따라감.
 */
interface Props {
  size?: number;
  color?: string;
...
export function HourglassHalfIcon({ size = 12, color, title }: Props) {
  return (
    <svg
      width={size}
      height={size}
